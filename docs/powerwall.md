...
Local JSON: `POST /api/login/Basic`, then `GET /api/meters/aggregates` and `GET /api/system_status/soe`. Gateway `percentage` is the raw pack (about 5% higher than the Tesla app). The panel converts that to the same 0–100 scale as the app / Fleet API. Powerwall 3 often has a weaker local API; use cloud if login fails.
...
The Note shows `POWERWALL` plus battery `%`, then solar (or **GRID** when solar is 0W) and house draw in **watts**. Grid is signed: import positive, export negative. The colour chip next to `%` uses the same live battery scale as Yarbo and Lymow: green at 60% or more, yellow from 40%, orange from 20%, red below that. Printed battery `%` writes at most every 2 minutes unless it jumps by 2% or more; solar, grid, and draw at most every 5 minutes, except the SOLAR/GRID flip which writes immediately. A late Gateway poll keeps the last good reading instead of flashing OFFLINE.
